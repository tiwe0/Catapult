...
func font_config_file_exists() -> bool:
    ...
    var config_file: String = Paths.config + str("fonts.json")
            
    return DirAccess.dir_exists_absolute(config_file)
...
func load_font_config() -> void:
    ...
    if DirAccess.dir_exists_absolute(config_file):
        var f = FileAccess.open(config_file, FileAccess.READ)
        if f:
            var j = JSON.new()
            var parse_result = j.parse(f.get_as_text())
            if parse_result == OK:
                result = j.data
            else:
...
func options_file_exists() -> bool:
    ...
    var options_file: String = Paths.config + str("options.json")
        
    return DirAccess.dir_exists_absolute(options_file)
...
func load_game_options() -> void:
    ...
    if DirAccess.dir_exists_absolute(options_file):
        var f = FileAccess.open(options_file, FileAccess.READ)
        if f:
            var j = JSON.new()
            var parse_result = j.parse(f.get_as_text())
            if parse_result == OK:
                _game_options = j.data
            else:
...
func _install_font(font_index: int) -> bool:
    ...
    var source := "res://fonts/ingame" + str(font_file)
    var dest: String = Paths.font_user + str(font_file)
