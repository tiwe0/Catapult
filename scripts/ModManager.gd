...
func parse_mods_dir(mods_dir: String) -> Dictionary:
    ...
    for subdir in FS.list_dir(mods_dir):
        ...
        var modinfo = mods_dir + str(subdir) + str("/modinfo.json")
        
        if FileAccess.file_exists(modinfo):
            
            var f = FileAccess.open(modinfo, FileAccess.READ)
            var json = JSON.new().parse_string(f.get_as_text())
            if json.error != OK:
...
func _install_mod(mod_id: String) -> void:
    ...
    if mod_id in available:
        ...
        if (mod_id in installed) and (installed[mod_id]["is_obsolete"] == true):
            ...
            var f = FileAccess.open(mods_dir + str(mod["location"].get_file()) + str("modinfo.json"), FileAccess.WRITE)
...
func retrieve_kenan_pack() -> void:
    ...
    var archive = Paths.own_dir + str(pack["filename"])
    if DirAccess.dir_exists_absolute(archive):
        ...
        for int_path in pack["internal_paths"]:
            FS.move_dir(Paths.tmp_dir + str(int_path), Paths.mod_repo)
            await FS.move_dir_done
        
        if Settings.read("install_archived_mods"):
            Status.post(tr("msg_unpacking_archived_mods"))
            FS.move_dir(Paths.tmp_dir + str(pack["archived_path"]), Paths.mod_repo)
            await FS.move_dir_done
        
        Status.post(tr("msg_kenan_install_cleanup"))
        FS.rm_dir(Paths.tmp_dir + str(pack["internal_paths"][0].split("/")[0]))
        await FS.rm_dir_done
